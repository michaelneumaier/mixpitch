...
<a href="{{ route('projects.download', ['project' => $track->id]) }}" class="btn btn-primary">Download</a>



                    @if (auth()->check() && auth()->user()->id === $track->user_id)
                        <a href="#" class="delete-btn" onclick="event.preventDefault(); document.getElementById('delete-project-form').submit();">Delete Project</a>
                        <form id="delete-project-form" action="{{ route('tracks.destroy', $track->id) }}" method="POST" style="display: none;">
                            @csrf
                            @method('DELETE')
                        </form>
                    @endif
