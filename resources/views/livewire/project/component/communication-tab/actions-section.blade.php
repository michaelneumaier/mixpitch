                    ...
                    @switch($action['type'])
                        ...
                        @case('unresolved_comments')
                            <div class="flex h-10 w-10 items-center justify-center rounded-full bg-orange-100 dark:bg-orange-900/50">
                                <flux:icon name="chat-bubble-left-ellipsis" class="h-5 w-5 text-orange-600 dark:text-orange-400" />
                            </div>
                            @break
                        @default
                    ...
                    @if (isset($action['action_url']))
                        <div class="mt-3">
                            ...
                            @elseif ($action['type'] === 'unread_messages')
                                {{-- For mobile: switch to messages section via Alpine --}}
                                <flux:button
                                    x-on:click="activeSection = 'messages'"
                                    variant="ghost"
                                    size="sm"
                                    class="lg:hidden"
                                >
                                ...
                                {{-- For desktop: scroll to messages (already visible) --}}
                                <span class="hidden text-sm text-gray-500 dark:text-gray-400 lg:inline">
...
@empty
    <div class="flex flex-col items-center justify-center py-8 text-center">
        <div class="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-green-100 dark:bg-green-900/50">
