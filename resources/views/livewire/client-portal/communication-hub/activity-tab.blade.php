{{-- Client Portal Activity Tab Content --}}
<div class="p-4">
                        ...
                        <span class="text-sm font-medium text-gray-900 dark:text-white">
                            @switch($displayType)
                                ...
                                @case('producer_message')
                                    {{ $event->user?->name ?? 'Producer' }} sent a message
                                    @break
                                @case('approval')
                                ...
                                @case('revision_request')
                                    ...
                                    @break
                                @case('status_change')
                                    @if ($event->status === \App\Models\Pitch::STATUS_READY_FOR_REVIEW)
                                        New deliverables ready for review
                                    @elseif ($event->status === \App\Models\Pitch::STATUS_COMPLETED)
                                        Project completed
                                    @else
                                        Status updated
                                    @endif
                                    @break
                                @case('file_uploaded')
                                    Files uploaded by producer
                                    @break
                                @case('work_session_completed')
                                    Producer completed a work session
