<div x-show="$wire.shouldShow">
    <flux:callout icon="clipboard-document-check" color="purple" class="mb-2">
                ...
                <div class="flex-1">
                    ...
                    <flux:heading size="sm" class="text-gray-900 dark:text-gray-100">
                        @if($requiredCount > 0 && $optionalCount > 0)
                            {{ $requiredCount }} {{ $requiredCount === 1 ? 'task' : 'tasks' }} to submit, {{ $optionalCount }} optional
                        @elseif($requiredCount > 0)
                            {{ $requiredCount }} {{ $requiredCount === 1 ? 'task' : 'tasks' }} to submit for client review
                                    ...
                                    <li class="flex items-start gap-2">
                                        ...
                                        <div class="flex-shrink-0 mt-0.5">
                                            @if($item['status'] === 'ready')
                                                <flux:icon.paper-airplane variant="solid" class="w-5 h-5 text-purple-600 dark:text-purple-400" />
                                            @else
