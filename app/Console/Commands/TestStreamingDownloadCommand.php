...
class TestStreamingDownloadCommand extends Command
{
    protected $signature = 'test:streaming-download {url} {--threshold=1 : Streaming threshold in MB}';

    protected $description = 'Test streaming download with custom threshold';
    ...
    public function handle()
    {
        ...
        $this->info("Testing streaming download with {$threshold}MB threshold: {$url}");

        // Temporarily override the config for testing
        config(['linkimport.processing.streaming_threshold_mb' => $threshold]);
        ...
        try {
            ...
            $this->info("Streaming threshold: {$threshold}MB");
            ...
            $this->info('Processing import job with streaming...');
            ...
            if ($import->status === 'completed') {
                $this->info('✅ Streaming download test completed successfully!');
