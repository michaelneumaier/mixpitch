...
class GenerateAudioWaveform implements ShouldQueue
{
    ...
    {
        try {
            // Call AWS Lambda function for audio processing
            $lambdaUrl = config('services.aws.lambda_audio_processor_url', null);
            
            // If Lambda URL is not configured, fall back to local estimation
            if (!$lambdaUrl) {
                Log::warning('AWS Lambda audio processor URL is not configured, using fallback method');
